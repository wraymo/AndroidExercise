...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator animatorscalex = ObjectAnimator.ofFloat(target, "scaleX",1, 2);
        ObjectAnimator animatorscaley = ObjectAnimator.ofFloat(target, "scaleY",1, 2);
        ...
        ObjectAnimator animatortrans = ObjectAnimator.ofFloat(target, "alpha", 1, 0.5f);
        animatortrans.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        animatortrans.setRepeatCount(ObjectAnimator.INFINITE);
        animatortrans.setRepeatMode(ObjectAnimator.REVERSE);
        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1, animatorscalex, animatorscaley, animatortrans);
        animatorSet.start();
    }
}
